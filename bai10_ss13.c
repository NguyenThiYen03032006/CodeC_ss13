#include<stdio.h>
void addAllItem(int arr[], int size){ // khoi tao mang
    for(int i=0; i<size; i++){
        printf("phan tu thu arr[%d] co gia tri: ",i);
        scanf("%d", &arr[i]);
    }
}
void showItem(int arr[], int size){ // in mang
    for(int i=0; i<size; i++){
        printf("phan tu arr[%d] co gia tri: %d \n", i, arr[i]);
    }
}
void addIem(int arr[], int *size, int index, int item){ // them phan tu
...
void fixItem(int index, int item, int size, int arr[]){ // sua phan tu
    if(index >= 0 && index < size){
        arr[index]= item;
    }
}
    ...
    do{
        ...
        switch(choose){
            ...
            case 3:
                ...
                break;
            case 4:
                // sua phan tu tai vi tri chi dinh
                if(flag!=1){
                    printf("ban chua khoi tao mang \n");
                }else{
